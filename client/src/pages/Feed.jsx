  ...
  return (
    <div className="container-app px-4 py-6">
      ...
      <div className="flex gap-1 mb-2 overflow-x-auto justify-center sticky top-14 z-10 bg-white py-1">
        <button onClick={() => setFilter('TOUS')} className={`px-2 py-1 rounded-full text-xs text-center ${filter === 'TOUS' ? 'bg-green-600 text-white' : 'bg-white border border-gray-200'}`}>Tous</button>
        {POST_TYPES.map(t => (
          <button key={t.id} onClick={() => setFilter(t.id)} className={`px-2 py-1 rounded-full text-xs text-center ${filter === t.id ? 'bg-green-600 text-white' : 'bg-white border border-gray-200'}`}>{t.label}</button>
        ))}
      </div>
