...
function JobCard({ job, users, onApply, onDelete, onUpdate, onBoostCV }) {
  ...
  // Vérifier si l'utilisateur a postulé (nouveau format objet ou ancien format string)
  const application = job.applicants?.find(a => 
    (typeof a === 'string' && a === user?.id) || 
    (a.userId && a.userId === user?.id)
  )
  const hasApplied = !!application
  const hasBoostedCV = application?.isBoosted === true
  ...
  const handleApply = () => {
    if (!hasApplied && user) {
      // Postulation AUTOMATIQUE avec message
      ...
      onApply(job.id, false, jobInfo)
      navigate(`/messages/${job.recruiterId}`)
    } else if (!user) {
      navigate('/login')
    }
  }
  ...
  const handleCvBoostSuccess = () => {
    ...
    const boostMsg = `Bonjour ${recruiter?.name},\n\nJe postule pour le poste de ${job.title} chez ${job.enterprise}.\n\nJ'ai boosté ma candidature pour être mis en avant.\n\nType: ${job.type}\nLieu: ${job.location}${job.salary ? '\nSalaire: ' + job.salary : ''}\n\nMerci de votre consideration.\n\n[CV BOOSTÉ - Candidat Premium]`
    onBoostCV(job.id, boostMsg)
    navigate(`/messages/${job.recruiterId}`)
  }
  ...
  return (
    <>
      <div className={`bg-white rounded-xl shadow-sm border p-4 ${isBoosted ? 'border-amber-300 ring-1 ring-amber-200' : 'border-gray-100'}`}>
              ...
              {hasApplied ? (
                <>
                  ...
                  {!hasBoostedCV && (
                    ...
                    >
                      <Bolt className="w-3 h-3" /> Booster 
                    </button>
                  )}
                  {hasBoostedCV && (
        ...
        {/* Message explicatif pour le boost de CV */}
